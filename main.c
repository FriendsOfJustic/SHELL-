...
void print_info(char *info)
{
  ...
  if(info[1]=='r'&&info[2]=='o'&&info[3]=='o'&&info[4]=='t'&&info[5]=='@')
...
char** line_cut(char *line)   //将从键盘输入的字符切割成指令
{
  char**result;
  char*cur;
  int sz=0;
...
void phrase_excute(char **phrase,int *return_value,char**env)   //程序替换的函数
{
  
  if(built_in_command(phrase,return_value,env))   //检查是否是内置命令
  {
    int flag=0;  //1 重定向  -1 管道  0 啥都没有
    int i=0;
    while(phrase[i])
    {
        if(strcmp(phrase[i],"|")==0)   //只有一次管道
          flag=-1;
        else if(strcmp(phrase[i],">")==0 || strcmp(phrase[i],">>")==0)
        {
          if(flag==-1)   //先管道后重定向
            flag=-i;     //这种情况只需要替换一下输出的文件描述符即可
          else 
            flag=1;    //只有一次重定向
        }
        i++;
